import { useState } from "react"
import { NavLink } from "react-router-dom"


const Navbar = () => {
    const [cartQty, setCartQty] = useState(0)
    return (
        <nav className="bg-dark fixed-top p-3">
            <div className="container">
                <div className="row">
                    <div className="col-12 col-md-4 d-flex justify-content-center align-items-center">
                        <h1 className="text-white">Shopping Cart</h1>
                    </div>
                    <div className="col-12 col-md-4 text-white d-flex justify-content-center align-items-center mt-1 mt-md-0">
                        <span className="text-white"><svg xmlns="http://www.w3.org/2000/svg" width="125" height="25" fill="currentColor" className="bi bi-cart-fill" viewBox="0 0 16 16">
                            <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z" />
                        </svg></span>
                        <span className="">Your cart has {cartQty} products</span>
                    </div>
                    <div className="col-12 col-md-4 d-flex justify-content-center align-items-center mt-3 mt-md-0">
                        <NavLink to="/cart"><button type="button" className="btn btn-dark btn-outline-success rounded-pill text-capitalize">Cart Details</button></NavLink>
                    </div>
                </div>
            </div>
        </nav>
    )
}

export { Navbar }